...
class CustomerServiceTest {
    @MockK lateinit var customerRepository: CustomerRepository                      // mockando o customerRepository(fake)
    @InjectMockKs lateinit var customerService: CustomerService                     // injetando a class que de fato queremos testar
    ...
    fun `should create customer`() {
        ...
        //when
        val actual: Customer = customerService.save(fakeCustomer)                   // nessa fase passamos exatamente o cenario que definimos na 'given' acima
        //then
    ...
    fun `should find customer by id`() {
        //given
        val fakeId: Long = Random().nextLong()                                       // vai gerar um id randômico p usarmos nos testes
        val fakeCustomer: Customer = buildCustomer(id = fakeId)                      // criamos o fake customer com a fun designada p tal, só q mudamos o id pra o id da 'val fakeId'
        every { customerRepository.findById(fakeId) } returns Optional.of(fakeCustomer)
        //when
        val actual: Customer = customerService.findById(fakeId)
        //then
        Assertions.assertThat(actual).isNotNull
        Assertions.assertThat(actual).isExactlyInstanceOf(Customer::class.java)     // se o customer em 'actual' é uma instância exata da class Customer
        Assertions.assertThat(actual).isSameAs(fakeCustomer)                        // se o customer salvo no DB é o mesmo fakeCustomer
    ...
    fun `should delete customer by id`() {
        ...
        val fakeCustomer: Customer = buildCustomer(id = fakeId)     // simula o retoro desse customer do DB pelo id achado pelo findById
